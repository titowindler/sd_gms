<?php
    class ClassService{
        ...
        public function addService($title, $type, $max, $price, $month, $year, $description, $schedule, $date_start, $trainer){
            $stm = "INSERT INTO `class_service`(`id`, `title`, `type`, `duration_month`, `duration_year`, `schedule_class`, `trainer_id`, `description`, `price`, `max_cap`, `num_ordered`, `date_created`, `status`) VALUES (null,'".$title."','".$type."','".$month."','".$year."','".$schedule."','".$trainer."','".$description."','".$price."','".$max."',0,'".$date_start."','active')";
        ...
        public function countClassService(){
            $stm = "SELECT COUNT(*) as count FROM class_service WHERE status='active'";
            $result = mysqli_query($this->conn, $stm);
